...
- **Stale slots and cancellation.** The command runs in its own process group. A slot is kept while its wrapper or any process of that group still runs, and reclaimed only when all have ended. A cancelled run (INT/TERM) signals the whole group, waits for it (then KILLs it after `EYE_HEAVY_TERM_GRACE`, default 30 s), and only then releases the slot; a normal finish also waits for the group (the PLAN-F4 residual of the review of 2026-09-25, corrected).
- **Verified.** The eight cases pass (`scripts/dev/heavy-slot.test.sh`; `evidence/planning/heavy-slot-test-20260925-cancellation.txt`): two of four concurrent, exit status passed through, TERM stops the command's process group, a SIGKILLed wrapper's running command keeps its slot, stale reclaim, a cancelled workload that shuts down late keeps its slot, the grace escalation, a finished command's lingering group member holds the slot. The control run of the cancellation case against the earlier limiter reproduces the defect (the waiting job entered ~3 s before the workload exited).
