# The NORDWERK demonstration: the complete story (baseline 2026-09-24, corrected 2026-09-25)
...
The provisional dates of every stage are in `audit/DELIVERY_PLAN.md` §3.2 (generated by the schedule model). Chapters overlap because the accounts work in parallel.

### Chapter 1. "Something material happened" (B23, B24; A1)
...
### Chapter 2. "What do we really know?" (B50, B40, B45, B51, B43, B46, B42, B48; A2)
...
### Chapter 3. "What might happen?" (B28, B29, B25, B27, B31, B30, B26; A1/A3)
...
### Chapter 4. "What does it mean for us?" (B32, B33, B53, B49, B55, B41, B44, B52, B54)
...
### Chapter 5. "Decide, commit, execute" (B34, B35, B36; A1)
...
### Chapter 6. "Agents do the legwork, under control" (B70–B78; A2/A3)
...
### Chapter 7. "The platform can be trusted" (B60–B67; A3)
...
### Chapter 8. "One product, for every role" (B80–B86; A3)
...
### Chapter 9. "It runs as a business" (B90–B94; A1)
...
### Chapter 10. "It is governed and runs anywhere" (B100–B112: construction moved forward from hardening and readiness)

- **B100:** A NORDWERK buyer double-submits a supplier-switch command over a flaky link; the second submit returns the first result. Every read declares its revision and freshness, and a stale projection answers as declared-degraded. A threshold change goes through the audited configuration surface, and a runtime exception expires into review.
- **B101:** The graph health view flags 40 orphan shipment nodes from a failed import and a rising projection lag, and opens a remediation task.
- **B102:** During a corridor alert storm, warning extraction keeps its latency budget while a bulk re-extraction is throttled; the economics ledger attributes the cost to NORDWERK's task.
- **B103:** A connector pod tries to reach an unregistered host; the egress gateway refuses it with a receipt. Internal hops run on mTLS with workload identities. A clock-quality breach refuses an authority change.
- **B104:** The Red Sea warning-storm load profile runs locally; the capacity view shows headroom and sheds bulk work first.
- **B105:** The primary database is killed during an approval; the replica is promoted and the approval completes once, reconciled.
- **B106 and B111:** The golden journey (observe → warn → decide → replay) runs on compose and on kind; the parity report shows identical semantics. The SaaS cell and placement service, and the private-cloud and on-prem site packages, are installed into local stand-ins.
- **B109 and B110:** Each layer's desired/observed manifests reconcile; an admission gate refuses an artifact that failed its exercises.
- **B107:** The product-governance workspace shows the capability baseline and its traceability. An expired exception holds a release. An acceptance record constrains a capability to its accepted scope.
- **B108:** The approval journey passes the automated and keyboard-only suites; preferences and timeouts persist.
- **B112:** A marketplace package's usage produces signed receipts against its licence.

Every one of these is shown on local stand-ins and stated as such. None closes a clause that needs the real infrastructure, auditor or counterparty.

### Epilogue (H1–H3: comprehensive verification; M3: the final walk; R1–R3: deployment readiness)

- **H1–H3** re-run the chapters' journeys under stress and at representative scale: drills, compose-vs-kind parity, containment and escape exercises. Independent assessment happens where named: a screen-reader user approves the package by keyboard with the WCAG auditor; the councils' constitution is recorded.
- **M3, the owner's walk:** one uninterrupted pass through Chapters 1–10 on `eye_demo`, from the closure bulletin to Decision Replay months later. Each scene's effects are cited from the ledgers. This is the demonstration of record for final acceptance.
- **R1–R3:** only with the external prerequisites:
  - NORDWERK and a second tenant in separate EU cells, one evacuated during maintenance; the install into a NORDWERK-owned private cloud on its IdP and keys;
