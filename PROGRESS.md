...
| Phase 3 — Enterprise Memory & Knowledge Graph | L3–L4 | **COMPLETE — merged `6914af03`** | Report: [PHASE3_REPORT.md](PHASE3_REPORT.md). Handoff: [PHASE3_PRODUCT_HANDOFF.md](PHASE3_PRODUCT_HANDOFF.md). Plan: [PHASE3_BUILD_PLAN.md](PHASE3_BUILD_PLAN.md) (C1–C7 frozen with the owner's eight resolver authority rules). Migration 0024. Entity registry, governed resolver, resolution queue with split, bitemporal provenance-bound edges, permission-aware search, Strategy Graph and invalidation propagation. Acceptance 43/43; integration 536/536; C18-era 297/297; upgrade check PASS. |
| Phase 4 — Prediction + Scenario Intelligence | L6–L7 | **IMPLEMENTED — functional correction review CLOSED at PR #38 head 98e7d9e6 (2026-09-06); unmerged behind the red C15 gate** | Report: [PHASE4_REPORT.md](PHASE4_REPORT.md). Handoff: [PHASE4_PRODUCT_HANDOFF.md](PHASE4_PRODUCT_HANDOFF.md). Plan: [PHASE4_DATA_READINESS_PLAN.md](PHASE4_DATA_READINESS_PLAN.md) (T1–T4, D1–D8, the owner's four source decisions of 2026-09-05). Migrations 0028–0031. Closed-range backfill on the governed REST poller; ECB EUR/USD activated and backfilled live (28 windows, 1999-01-04 → today); series assembly through the known-at path, a governed retrieval per reader on every read (no cache), disclosing unreadable evidence; seasonal naive and Holt-Winters with the model on a leash bound to the APPLICABLE backtest; backtests in retrospective and historical knowledge modes; controls inherited evidence → forecast → scenario → warning; one warning per flip, owed until raised, controls from the exact cited evidence version; issuance timed against the declarer's decision deadline (missed decisions recorded) and the response timed separately on the warning's own clock (replay or live); outcomes never scored before the target nor from a stand-in the attested publication calendar does not justify. ECB at 30 days: T1 met (85%) and T2 NOT met (skill −1.6%) **retrospectively**; historically it cannot validate on a backfilled vintage. T3 unmeasured. The corridor forecast is a replay demonstration that cannot be validated and says so. Two bounded correction passes against the Codex reviews of 737ca81a (report §10) and aec404c1 (report §12). PortWatch stays in replay pending the IMF's answer; UN Comtrade untouched; nothing purchased. |
| Phase 5 — Digital Twins + Simulation | L5, L8 | **IMPLEMENTED on `phase5-twins` (M1–M7); functional correction review CLOSED at PR #41 head 48f43bed (2026-09-07) after three consolidated passes (migrations 0035–0037: the Codex reviews of f66a958d, 1a05af89 and 37bf4103); unmerged behind the red C15 gate** | Plan: [PHASE5_BUILD_PLAN.md](PHASE5_BUILD_PLAN.md). Report: [PHASE5_REPORT.md](PHASE5_REPORT.md). Baseline: Phase 4 at PR #38 head 879ce2d8. |
