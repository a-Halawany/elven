...
--config vitest.int.config.ts`. Full integration currently **271/271**, acceptance **58/58** (per-run journal isolation closed; no manual cleanup). Migrations now **0001–0021** (0001–0020 immutable; 0021 = C13 surface closure). **Both DB paths proven at 0019**: forward upgrade from a real 0001–0012 database seeded through the historical bootstrap port (19 migrations, zero data loss, pre-existing rows/hashes/chain/digests byte-identical, chain verifies with no rehash, full suite 251/251 on the upgraded DB) and virgin install 0001–0019 (251/251 + acceptance 44/44). **External independent
