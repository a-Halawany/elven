...
for (const s of rows) {
  ...
  console.log(`  runtime        : scheduler ${a.runtime.scheduler_enabled ? 'ENABLED' : 'disabled'} · worker ${a.runtime.worker_running ? 'RUNNING here' : 'not running here'} · redis scheduler ${a.runtime.redis_scheduler.state === 'present' ? `present, every ${a.runtime.redis_scheduler.every_seconds} s, next fire ${fmt(a.runtime.redis_scheduler.next_at)}` : a.runtime.redis_scheduler.state ?? 'absent'}`);
  console.log(`  automatic runs : ${a.attempts.scope ?? 'newest'} ${a.attempts.total ?? ''} · ${a.attempts.finished} finished · ${a.attempts.failed} failed · ${a.attempts.cancelled} cancelled · ${a.attempts.budget_exceeded} budget · ${a.attempts.faulted ?? 0} faulted · ${a.attempts.refused} refused`);
