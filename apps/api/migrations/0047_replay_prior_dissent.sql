-- 0047 · Phase 6, P6-M7 — the replay's decided layer also carries the dissent recorded on the
-- package's EARLIER versions before the decision (F3: dissent is never removed by a change of
-- choice; Act VI showed it recorded on version 1 while version 2 was committed). Forward
-- migration: decision.replay_layers is replaced whole; the content digest of a replay now
-- covers prior_dissent as well, so replays recorded before this migration keep their digests
-- and new replays of the same package carry a different one.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- dissent recorded on the package's EARLIER versions before the decision: the choice changed, the dissent did not vanish (F3)
