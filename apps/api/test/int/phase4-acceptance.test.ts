...
describe('D1–D8 (database / API) — forecasts, scenarios, warnings, outcomes, propagation', () => {
  ...
  it('D8 · every prediction table is under FORCE row-level security, and the projections rebuild from their logs', async () => {
    ...
    expect(rows.length).toBe(19); // 12 of 0029–0037, the scenario kind vocabulary of 0058, the warning level versions and derivations of 0061, the warning suppressions of 0066, the forecast fitness assessments and the scenario coherence checks of 0081 (B21); + the scenario branch requests of 0084 (B23)
    for (const r of rows) {
