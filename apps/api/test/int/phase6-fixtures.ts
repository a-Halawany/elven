...
export function decisionCalls(h: Phase4Harness, w: DecisionWorld) {
  ...
  /** The stored outputs are read under the purpose they were produced for: 'briefing' for a briefing task, 'decision' otherwise. */
  const listAgents = (as = w.executive, purpose = 'decision') => w.exec.listAgents(h.req(as, 'agent.read', 'AGT', null, purpose), T(), D()) as Promise<{ agents: Array<Record<string, unknown>>; runs: Array<Record<string, unknown>>; planner: Record<string, unknown> }>;
  const runAgent = (agentId: string, payload: Record<string, unknown>, as = w.owner, purpose = payload['task'] === 'briefing' ? 'briefing' : 'decision') => w.exec.runAgent(h.req(as, 'agent.trigger', 'AGT', agentId, purpose), T(), D(), agentId, { payload: payload as never }) as Promise<{ run: { runId: string; outcome: string; spent: Record<string, unknown>; stopReason: string | null; refusals: Array<Record<string, unknown>>; outputs: Record<string, unknown>; escalatedTo: string | null } }>;
