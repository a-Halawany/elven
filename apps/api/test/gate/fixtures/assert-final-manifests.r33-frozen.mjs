 ...
 *
 * A byte copy of `scripts/gate/assert-final-manifests.mjs` at commit 18b563d — the C16-R3.3
 * verifier. It constrained the step SET, cross-checked step receipts against bindings,
 * recomputed cache aggregates and parsed each SBOM. Independent review then showed seven
 * coordinated false passes it still accepts, all with the same root cause: an expectation
 * derived from the evidence being checked. Most sharply, the expected image-step set was
 * derived from `digest_pinned_images` — a manifest field — so replacing both configured images
 * with one attacker-chosen digest and deleting the second step, its files and its bindings
 * produced a perfectly self-consistent, perfectly accepted package.
 *
 * It exists so the C16-R3.4 mutation controls can EXECUTE the defective behaviour.
 *
 * Two mechanical edits: the `import.meta.url` main-module guard is removed so importing this
 * file cannot execute a CLI, and its contracts import is repointed at the frozen copy beside
 * it so this file keeps verifying the way R3.3 did even as the live contracts move on.
 */
...
} from './final-assertion-contracts.r33-frozen.mjs';
