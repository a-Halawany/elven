...
export class SeriesService {
  ...
  private async load(
    ...
    // The parse depends on the parser, the field AND the selector: two series
    // over the same evidence (two chokepoints in one PortWatch page) must never
    // share an entry.
    const key = `${series.parser_ref}|${series.value_field}|${series.selector ?? ''}|${v.object_id}@${v.object_version}`;
    ...
    try {
      ...
      // Withdrawn, tombstoned or unverifiable bytes are not served and yield no
      // point: an absent observation, never an invented one.
      return null;
    }
