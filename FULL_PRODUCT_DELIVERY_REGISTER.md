...
Units: 3884. Source rows referenced: 6264 (6264 resolve to register rows of 6264). **Mandatory units UNFINISHED for full-profile acceptance: 3535** = 3196 in status open + 339 verified on the local profile only while applying to all profiles (S7: not accepted until every agreed profile is verified). Mandatory units classified not-applicable (reconciled individually, see audit/UNIT_REPAIRS.md): 0. Problems: 0.
...
| Package | unfinished mandatory units | of which verified:local (profiles=all) |
|---|---|---|
| P1 | 124 | 0 |
| P2 | 118 | 0 |
| P3 | 141 | 1 |
...
Reading the unit table: 3884 units decompose the 6264 rows (every row referenced). Mandatory units unfinished for full-profile acceptance: 3535 = 3196 in status open + 339 verified on the local profile only while applying to all profiles; none is `done`; the local statuses are authored assignments bound to named cases (59 that named only a module were set back to open in `audit/UNIT_REPAIRS.md`), not independent verification. No mandatory unit is classified not-applicable any more: the twelve were reconciled individually into **seven open document-class obligations and five documentary rules** (`audit/UNIT_REPAIRS.md` §1; an earlier summary said eight and four — the prose was corrected to the data). The counts say what is owed, not what fraction is delivered.
...
| Database/host clock skew | `open` (observed, cause not investigated) | on 2026-09-10 the database container's clock ran 28 min 15 s behind the host during the morning (replay ticks failed with "recorded_at is in the future"); measured 0 s at 16:35 UTC; a P7-D operations item: clock discipline and a health signal exposing database time |
...
| CP-4 · Coherent recovery — **the planned work is done** (2026-09-10, `docs/ops/evidence/restore-drill-20260910T223824Z.md`, 68/1 checks; guards 51/51) | none | **Build identity** replaces the timestamp heuristic: the bundle is built from the committed tree at a recorded SHA with the committed lockfile and carries the artefact, and the restore refuses a build whose deterministic dist digest differs — it fired for real in the drill and recorded a source-to-target upgrade instead of starting silently. **Encryption**: AES-256-GCM per file under a per-bundle key wrapped by PBKDF2-HMAC-SHA512 (600k) with a tag verified before any ciphertext is read; this host's `openssl enc` refuses AEAD and `age`/`gpg` are absent, so the AEAD comes from `node:crypto` (a host without node needs `bundle-crypto.mjs`) — recorded, not worked around. **Non-empty degraded journal** produced by the code's own writer in isolation (a revoked EXECUTE grant on the login path → three `audit_unavailable` records and three governed incidents), restored (`/readyz` degraded, same `degradedSince`, journal replayed) and reconciled to `ok`. **Coherent capture**: a recorded boundary (snapshot xmin/xmax, audit heads, newest manifest, vault listing digest) reconciled row by row on restore, plus quiescence for the isolated source; both bundles `capture.stable: true`. **Scheduler reconstruction with collection enabled**: Redis empty → 5/5 eligible entries and 97 attempts rebuilt from the database alone, exactly one tick served and recorded. OPEN: the live containers' recreation (CP-4a below) |
| CP-4a · The live containers' recreation onto the pinned images | CP-4; the two credential findings below | not yet done. Two conditions found by the drill must be resolved in the same governed operation: (1) the `platform-admin` principal refuses the `EYE_TEST_ADMIN_PASSWORD` of the current `.eye-local/env` (`EYE-IDN-002`), so a restored deployment cannot serve the governed read that proves it — either the password that matches the stored credential, or a governed rotation of that principal; the database roles were realigned (7/9 authenticate, 2 are NOLOGIN by design) but the principal was not. (2) `eye-redis` has been `unhealthy` for 40 hours: the running server's `requirepass` matches the CURRENT env file (verified by an authenticated PING), while the healthcheck variable baked into the container at creation does not — the application is unaffected, the healthcheck is stale, and a recreation rebuilds both from one env read. That a red healthcheck surfaced nowhere for 40 hours is itself a P7-D observability row |
