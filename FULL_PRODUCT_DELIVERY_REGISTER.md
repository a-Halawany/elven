...
Units: 3891. Source rows referenced: 6264 (6264 resolve to register rows of 6264). **Mandatory units UNFINISHED for full-profile acceptance: 3542** = 3202 in status open + 339 verified on the author's harness only (verified:local) + 1 verified on the hosted chain only (verified:ci); 0 of them carry at least one accepted profile leg. Mandatory units verified on every leg they apply to (verified:all): 0. (S7: a unit is finished only when every leg of its profile vector carries its own signed evidence; a harness run on any machine is not a leg.) Mandatory units classified not-applicable (reconciled individually, see audit/UNIT_REPAIRS.md): 0. CAP bindings of Volume 9: 89 = 71 defined in Volume 8 Appendix A + 18 resolved by alias (18 alias rows in audit/CAP_ALIASES.md); unresolved 0. Problems: 0.
...
S7 in the accounting: `audit/summarise-units.mjs` counts a mandatory unit as UNFINISHED unless it is verified on every leg of its profile vector — the units in status `open` PLUS the units verified on one artefact only (`verified:local`, the author's harness; `verified:ci`, the hosted chain on a fresh database), neither of which is a deployment leg; single-profile progress is shown apart and is never accepted cross-profile. Since batch B4 the profile column is the LEG VECTOR (saas | private | onprem, plus disconnected | air-gapped for the 80 capabilities with an offline obligation; 23 units that name one profile carry one leg) and `legs_verified` records, per leg, the pointer to the signed per-leg result; `verified:all` is refused unless every leg is accepted, and an accepted leg on an open unit is refused. No leg of any unit is accepted at this head.

Reading the unit table: 3891 units decompose the 6264 rows (every row referenced). Mandatory units unfinished for full-profile acceptance: 3542 = 3202 in status open + 339 verified on the author's harness only + 1 verified on the hosted chain only (AU-PRD-0021 at `1a99784`); the seven units added by CP-6 B1 and B2 (AU-MEM-0108–0111, AU-PRD-0061–0063) are open until the hosted run at their implementing head is green — the split was 3,535 = 3,195 + 339 + 1 before they were allocated. Legs accepted: none (saas 0 of 3525, private 0 of 3524, onprem 0 of 3531, disconnected 0 of 80, air-gapped 0 of 80). None is `done`; the local statuses are authored assignments bound to named cases (59 that named only a module were set back to open in `audit/UNIT_REPAIRS.md`), not independent verification. No mandatory unit is classified not-applicable any more: the twelve were reconciled individually into **seven open document-class obligations and five documentary rules** (`audit/UNIT_REPAIRS.md` §1; an earlier summary said eight and four — the prose was corrected to the data). The Volume 9 capability bindings resolve completely: 89 = 71 defined in Volume 8 Appendix A + 18 by the versioned alias table (`audit/CAP_ALIASES.md`, B5). The counts say what is owed, not what fraction is delivered.
...
| CP-6 · Audit second pass complete — **batches defined 2026-09-11 (`audit/CP6_BATCHES.md`); B3 (the eight scenario kinds, 0058; demonstration corrected, 0059), B1 (the `CorrectionApplied` consumer, 0060), B2 (warning levels, 0061) implemented and exercised on the demonstration; B4 (profile legs, SLO catalogue) and B5 (CAP aliases) applied to the audit; units verified locally, hosted verification pending** | none (read-only) | acceptance units for every capability group (`audit/acceptance-units/`), every `partial` row's remaining work verified against the code, the owner's four choices (R-4) recorded, `audit/SUMMARY.md` regenerated; no completion percentage |
