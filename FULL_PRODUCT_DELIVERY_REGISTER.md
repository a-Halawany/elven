...
| A scheduled walk that stalls mid-page (observed 2026-09-10, `SOURCE_INTEGRATION_STATUS.md` §9.11.7 item 3) | `open` — owner P1 (collection worker); cause not yet diagnosed | the run admitted six pages and 424 rows of the seventh and stopped: no further event, process alive at 0 % CPU, no terminal event. The lease expiry now releases the source (0051) and a displaced run can no longer admit after a takeover (0056), so the consequence is bounded; the observation — a job that stops making progress is neither finished nor failed and nothing says so — stands until the worker reports it. |
| The outbox publisher ended the API process during a correction (`capability denied: mode publish required (context is none)` from `outbox_lease`, §9.11.7 item 2) | `open` — owner P3/P7-D (outbox); cause not yet diagnosed | the capability is issued and then leased and the lease finds no context; a background publisher that cannot publish must report and retry, not exit the process. Reproduction at the harness is the next step. |
| `phase5-twins`: three cases refuse `consumption.weekly` as stale under full-suite load on the long-lived local harness database (`test:int:all` 803/806 locally; 15/15 in isolation; green on CI's fresh database) | `open` — time-sensitivity to characterise (owner P5) | fresh-database green does not erase it; the local `eye` database is also the one the reproduction probes of 2026-09-11 left eleven orphaned `observation.run.start` success audits in (F05's global invariant), so a fresh local harness database is the next step for both. |
...
| CP-4a · The live containers' recreation onto the pinned images | CP-4 (done); **the arm64 risk decision** | not yet done. The two credential conditions the fourth drill found are **resolved** (2026-09-11): `platform-admin` and the eleven demo personas were rotated to the current `.eye-local/env` value through `POST /v1/auth/rotate`, using the credential recovered from the pre-regeneration bundle's `config/env` (twelve audited `identity.credential_rotated` events; nothing reset, nothing printed); the Redis server accepts the current password and only the container's baked healthcheck variable is stale, which the recreation from one env read resolves. The rollback point is bundle `20260911T112927Z` (format /2, from `1c8242e`). What still blocks it is that this host runs the `linux/arm64` child of the pinned index, whose findings have no attributable approval (`docs/images/ARM64_RISK_DECISION.md`): the recreation waits for that decision, then proceeds with verified digests, health, service access and process protections recorded. |
