...
Units: 3884. Source rows referenced: 6264 (6264 resolve to register rows of 6264). **Mandatory units UNFINISHED for full-profile acceptance: 3535** = 3195 in status open + 339 verified on the local profile only + 1 verified on the hosted chain only, while applying to all profiles (S7: not accepted until every agreed profile carries its own evidence; an owner or a date never closes a unit).
...
| Package | unfinished mandatory units | of which verified:local (profiles=all) | of which verified:ci (profiles=all) |
|---|---|---|---|
| P1 | 124 | 0 | 0 |
| P2 | 118 | 0 | 0 |
| P3 | 141 | 1 | 0 |
...
S7 in the accounting: `audit/summarise-units.mjs` counts a mandatory unit as UNFINISHED unless it is verified on every profile it applies to — the units in status `open` PLUS the units verified on one profile only (`verified:local`, the author's harness; `verified:ci`, the hosted chain) while applying to all profiles; single-profile progress is shown apart and is never accepted cross-profile.

Reading the unit table: 3884 units decompose the 6264 rows (every row referenced). Mandatory units unfinished for full-profile acceptance: 3535 = 3195 in status open + 339 verified on the local profile only + 1 verified on the hosted chain only (AU-PRD-0021 at `1a99784`), all applying to all profiles; none is `done`; the local statuses are authored assignments bound to named cases (59 that named only a module were set back to open in `audit/UNIT_REPAIRS.md`), not independent verification. No mandatory unit is classified not-applicable any more: the twelve were reconciled individually into **seven open document-class obligations and five documentary rules** (`audit/UNIT_REPAIRS.md` §1; an earlier summary said eight and four — the prose was corrected to the data). The counts say what is owed, not what fraction is delivered.
...
| CP-6 · Audit second pass complete — **batches defined 2026-09-11 (`audit/CP6_BATCHES.md`); B3 (the eight scenario kinds) implemented, migration 0058, on the demonstration** | none (read-only) | acceptance units for every capability group (`audit/acceptance-units/`), every `partial` row's remaining work verified against the code, the owner's four choices (R-4) recorded, `audit/SUMMARY.md` regenerated; no completion percentage |
