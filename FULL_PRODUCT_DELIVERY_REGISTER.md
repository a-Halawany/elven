...
| CP-4 · Coherent recovery adopted — guards physical-path and fresh-destination (38/38 probes), three drills (37/37, 44/44 against the PUBLISHED digests with the compose protections observed) | none | OPEN parts: bundle encryption at rest before any bundle leaves the host; a non-empty degraded-journal restore; a consistent capture boundary across database/vault/journal/config (or a bounded reconciliation procedure); scheduler reconstitution proven with collection enabled; the live containers' recreation onto the pinned images recorded |
