...
Reading the unit table: 3904 units decompose the 6264 rows (every row referenced). Mandatory units unfinished for full-profile acceptance: 3555 = 3179 in status open + 339 verified on the author's harness only + 37 verified on the hosted chain only (AU-COM-0058 `verified:local` by B15; AU-TWN-0033 `verified:ci` by B18 at `4cea858`, ci 35163213469; AU-MEM-0068 and AU-MEM-0083 `verified:ci` by B20 at `1f6d04c`, ci 35779940271 attempt 2) (the B11 units AU-MEM-0059 and AU-MEM-0061 `verified:ci` at `1e3e4be`, ci 34869384873; the B12 units AU-MEM-0062 and AU-INF-0791 `verified:ci` at `97576c3`, ci 35004457633; the B13 units AU-IDP-0179, AU-IDP-0180 and AU-COM-0060 `verified:ci` at `3a5a181`, ci 35032929806 — 1016/1016 in 63 files on a fresh database with `phase6-retention-b13` 10/10, C19 35032929929; the earlier 27 CI units: (the seven CP-6 B9 units AU-DP-0176, AU-MEM-0121, AU-PRD-0064, AU-MEM-0065, AU-MEM-0060, AU-DP-0091 and AU-EXO-0051 at `36ce748`, ci run 34726253751, 929/929 in 56 files on a fresh database, C19 34726253762; AU-PRD-0021 at `1a99784`; six of the seven CP-6 B1/B2 units at `5118376`, ci run 34647461349; AU-MEM-0109 at `94d66f8`, ci run 34655152733; the six CP-6 B6 units AU-MEM-0112 to AU-MEM-0117 and AU-MEM-0029 at `fcbdefc`, ci run 34663012651; the two CP-6 B7 units AU-MEM-0118 and AU-MEM-0119 at `a852c65`, ci run 34693808238; the three CP-6 B8 units AU-DP-0175, AU-MEM-0120 and AU-MEM-0039 at `661c2fb`, ci run 34705325289, 890/890 in 53 files on a fresh database, C19 34705325299). The total rose from 3,542 to 3,555 across B6, B7, B8 and B9 by allocation of thirteen new units (B9: AU-DP-0176, the automatic re-derivation; AU-MEM-0121, the governed floor act; AU-PRD-0064, the scenario review's harness leg — all three `verified:ci` by the hosted run at `36ce748`; four units moved open → `verified:local` by B9: AU-MEM-0065, AU-MEM-0060, AU-DP-0091, AU-EXO-0051 (AU-MEM-0059 moved and moved back by the adversarial review — archive and customer-export actions do not execute); B8: AU-DP-0175, ordered publication across ticks, lease recovery and processes with one server per domain; AU-MEM-0120, a replay reaching the retained history with the declared floor — both `verified:ci` at `661c2fb`, with AU-MEM-0039 moved from open to `verified:ci` as its last two conditions gained fault cases), not by regression; AU-MEM-0041, AU-MEM-0114, AU-DP-0071, AU-DP-0041 and AU-MEM-0031 carry their delivered and remaining clauses in their own prose. No leg of any unit is accepted; nothing here is an acceptance.
...
| UN Comtrade | `authorized, not activated` | the owner's authorization of 2026-09-12 — existing keys within the source's permissions and the existing budgets and cadences (`SOURCE_INTEGRATION_STATUS.md` §10); no activation act performed; the automated-use terms still to be read and recorded before any activation |
...
| CP-6 · Audit second pass complete — **batches defined 2026-09-11 (`audit/CP6_BATCHES.md`); B3 (the eight scenario kinds, 0058; demonstration corrected, 0059), B1 (the `CorrectionApplied` consumer, 0060), B2 (warning levels, 0061) implemented and exercised on the demonstration; B4 (profile legs, SLO catalogue) and B5 (CAP aliases) applied to the audit; the seven B1/B2 units `verified:ci` at `5118376`; Codex's two findings (recovery of interrupted attempts, 0062; evidence-backed S7 accounting) corrected and hosted green at `94d66f8`; B6 (the `GraphChanged`/`MemoryCorrected` subscriptions and their six consumers, 0063, AU-MEM-0112–0117) implemented, exercised on the demonstration and `verified:ci` at `fcbdefc` (ci 34663012651, 852/852 on a fresh database); Codex's third finding (a retrieval retry cleared a mismatch without a second check) reproduced at the database/queue boundary and corrected by B7 (0064: unresolved work stays unresolved; the declared partition and sequence; failure classes; telemetry; legal holds; AU-MEM-0118/-0119) and `verified:ci` at `a852c65` (ci 34693808238, 867/867 on a fresh database); Codex's two B7 event-delivery findings (an overtaken failed publish within one publisher process; a replay omitting the history a registration skipped) reproduced at the database/queue boundary and corrected by B8 (0065: the lease partition-ordered across ticks, lease recovery and processes; the served point moved by a replay; one server per domain; the last two AU-MEM-0039 conditions; the four flows' telemetry; the interface register and the retention floor; inferred relationships reassessed on evidence or model change; warnings and briefings in the impact set; AU-DP-0175/AU-MEM-0120) — **`verified:ci`** at `661c2fb` (ci run 34705325289, 890/890 in 53 files on a fresh database, the upgrade proof with 0065; C19 34705325299); exercised on the demonstration (`evidence/cp6/act-b8.txt`, with the demonstration database's recorded 0064 reconciliation); B9, B10 and B11 (with Codex's closures) merged on `main` through #45–#48 (main `41d4a26`); **B12** (0072: the governed restore-to-hot port, the cold-tier manager — budget, ordering, retries, escalation, observable state, the restore window — and the sweeper's walk of the archive root; AU-MEM-0062, AU-INF-0791 `verified:ci` at `97576c3`; L3-C08, DZ-18, DAT-ST-06 `implemented` on the local profile) merged with #49 (main `2d760e4`, chain green) on Codex's bounded functional review; **B13** (0073: the governed schedule retirement; the customer export's delivery — the package as one verifiable tar downloaded under audit, the key-based signature `eye-customer-export/2` by a key declared by reference, declared destinations — a transfer station and an https endpoint — with the delivery recorded whole, the recipient's receipt and acknowledgement closing the exchange; AU-IDP-0179, AU-IDP-0180, AU-COM-0060 `verified:ci` at `3a5a181`; DPD-19, LR-23 `implemented`; the exchange rows `partial`) implemented and exercised on the demonstration (`evidence/cp6/act-b13.txt`: the B12-declared schedule retired with its history, the NORDWERK export signed with the demonstration key, downloaded and verified offline, delivered to the isolated transfer station and acknowledged, the production path's gate, the revocation), merged with #50 (main `be72aa8`, chain green — the records-head C19 green on attempt 2 after GitHub's artifact endpoint answered 504 on attempt 1) on Codex's bounded review; **B14** (0074: the https exchange proven over TLS against a synthetic recipient — acknowledged in one act, the stale receipt held as evidence and not acknowledged (Codex B13-F1 corrected: the receipt bound to its delivery at the initial record as at the acknowledgement), every wrong answer classified; the destination's trust anchor; the revocation notice to every destination that received a package — the station told and its confirmation collected, the https endpoint told in the revoke act, a failed notice retried, a refusal recorded mismatched; no unit moves, ES-29-005 `partial`) implemented and exercised on the demonstration (`evidence/cp6/act-b14.txt`: the anchor recorded, the https path through the production egress refused by the vetting on this host — the positive exchange the harness's, a public recipient the activation step — the station exchange and its revocation notice acknowledged, a refusal mismatched); **B15** (0075: the relationship closure in the export — the claims by lineage, the graph's edges and the entities they connect, under the ceiling, in links.json inside the signed chain, verified offline and by the product; the streamed archive — the stream route, the streamed station write and https delivery, the constant-memory verifier, the customer's fetch tool; a 272 MB package above the in-memory ceiling on the harness; AU-COM-0058 `verified:local`; DP-47-003 `implemented`) implemented and exercised on the demonstration (`evidence/cp6/act-b15.txt`: the NORDWERK export carrying its derived knowledge, streamed through the real HTTP path and to the station); **B16** (0076: the governed import — a partner as a public key with an intake contract, the package quarantined and checked sixteen ways, approved by another principal, admitted under new ids with the origin's identity as provenance; the NORDWERK export → import → re-export round trip; the versioned closure — Codex B15-F1 closed; held recipients — Codex B14-F1 closed) merged with #53 (main `ace1f9e`); **B17** (0077: imported knowledge published to the importing domain's subscribers; the origin's revocation propagated into the importing domain and executed there with the signed notice; imported claims not reviewable) merged with #54 (main `28e18b5`, chain complete) — both under the owner's word on Codex's bounded B16/B17 review; **B18** (0078, PR #55: Codex's B17-F1 corrected first with the refuters' four gaps; the lifecycle announced — ten interface rows bound, the register 36/14/0 — and the withdrawal → invalidation → reopen chain; the working domain of a tenant-homed principal and the hosted browser walks of the retention and memory workspaces; AU-TWN-0033 `verified:local`, then `verified:ci` at `4cea858`) implemented and exercised on the demonstration (`evidence/cp6/act-b18.txt`); **B19** (0079, PR #56 stacked on #55: the source-derived memory records — a record derived by a person from a claim version or a warning with its provenance, inherited controls, the review/lifecycle/evidence/clearance gates, the basis followed and the deletion of its evidence paused; MEM@v2; AU-MEM-0065's last clause closed on the harness, the gate and the demonstration) implemented and exercised on the demonstration (`evidence/cp6/act-b19.txt`); **B20** (0080, on `phase6-b20` over B19's records head `3ea676d`, stacked on #56: the index tier — no lexical/vector index exists, the index tier is the six derived projections; the partitions with a derived watermark on every one of the twelve exploration and memory reads, the symmetric check that withdraws a drifted, poisoned, missing, policy-inconsistent or outdated-representation partition where the JOIN-only check passed poisoned and missing rows, the operator's withdrawal and the rebuild writer from the log, the canonical record and the claim's lineage with the unrebuildable named and the held poison refused, the labelled last-valid reads and the constrained traversals, the memory content tier's metadata-only fallback for the canonical read (the evidence bytes out by the A7 doctrine), the deletion paused while a projection the safe scope reads is withdrawn, the briefing's flag; AU-MEM-0067/-0068/-0083 gain their evidence and stay `open` until the hosted run binds, AU-MEM-0070 stays `verified:local` with its evidence corrected; L3-I02's `bound_to` gains the clause, the register 36/14/0) implemented and exercised on the demonstration (`evidence/cp6/act-b20.txt`: the edges projection withdrawn on a representation review, the explore walk served from the log and constrained, the approved deletion paused, the briefing undegraded by the projection, the partition restored — ALL SCENES HELD, 39 checks); #55 merged to `main` as `e70f90f` on 2026-09-22 under the owner's word on Codex's bounded B18/B19 review — its ci red on the C15 patched-image recheck step by design (compatible fixed official images exist; the C15 gate itself green), its C17 finalize skipped, preserved as it ran; #56 held for the C15 return (`maintenance/c15-return-to-official-2026-09`, the draft PR #57) and the owner's word; then, on 2026-09-23 under the owner's authorization on Codex's B20 review, #57 completed (the six SCX re-issues approved; the recheck's return transition) and merged (`main` `870b212`), #56 retargeted and merged (`main` `6212c5b`, its chain fully green — the cumulative chain covering #55 and #57), #58 retargeted and merged (`main` `e180b18`; its post-merge chain green and its anchor published — the merge made while attempt 2's build-test was pending, the integrator's error recorded; the C19 causal rule against partial re-runs recorded in the runbook); **B21** (0081, on `phase6-b21` over B20's records head `13ed40c`, the PR with base `main`: fitness, coherence and challenge — one fitness vocabulary set only by a recorded act whose measures the port computes, the twin validated under the declared operating envelope with the owner refused by the separation of duties and the envelope enforced at the run, the forecast assessed under a versioned rule and announced to the consumers, the coherence check that admits a failed scenario and gates it out of simulation, promotion and decision-active use, the challenge decided by someone else and resolved by a governed re-run, a dismissal or an invalidation with the trigger `challenge`, the promotion by a reviewer (OBJ-29); the register 40/10/0 with L5-I05, L6-I03, L7-I04 and L8-I04 bound; Codex's B20-F1 corrected first — the withdrawn-mode memory reader's canonical statements under the content-tier boundary; the vault clause of AU-MEM-0067 delivered for the root-unreachable class (`custody.retrieval_degraded`, 200 metadata-only, `EYE-DEG-001`) and bounded at the per-object class by A7 with that custody row made durable; the rehearsal wedge — a governed write nested inside another deadlocking on `ctx.build`'s sweep, phase 5's making — fixed where the act reached it, the residual sites and the systemic remedy the owner's call; AU-MEM-0067 and the nine group-b units AU-TWN-0014/-0015/-0018/-0031, AU-PRD-0012/-0014/-0026/-0029/-0030 gain their evidence and stay `open` until the hosted run binds) implemented and exercised on the demonstration (`evidence/cp6/act-b21.txt`: the twin validated fit, the corridor forecast assessed unfit with its six deliveries, the duplicate-branch scenario admitted incoherent and refused for simulation, the challenge re-run and dismissed, the control promoted, the archive root made unreachable and the cold record answering metadata-only — ALL SCENES HELD, 54 checks); #59 merged 2026-09-24 (main `5165a97`, its chain complete); B22 (0082: the capability-nonce sweep that never waits — the residual nested-write wedge proven and gone; 0083: the attention policy — versioned, set by a named human, AttentionPolicyChanged@v1 — the transparent engine, the queue with acknowledgement, suppression, escalation and re-evaluation, the registered consumers of ObservationRecorded, SourceHealthChanged and the proposed claims with the invalid-event quarantine, the source-health markers, the transformation plan, a policy change as a recorded cause to reopen; the register 44/6/0; fourteen units gain evidence and stay open) implemented and exercised on the demonstration (`evidence/cp6/act-b22.txt` — the queue built from the domain's history, the PortWatch source's products marked and cleared, the B18 package reopened on the policy cause, overdue warnings escalated — ALL SCENES HELD); the demonstration's containers returned to the official image pins (`docs/ops/evidence/live-recreation-20260924T164303Z.md`); **B23** (0084, on `phase6-b23` stacked on the planning PR #61: the six partial interfaces bound — MaterialChangeRaised@v1 and ReviewConvened@v1 as events routed by the attention subscriber, governed reviews, the Acquire stream form, the purpose-bound RetrieveContext, the atomic idempotent CommitGraphRevision, the versioned BranchScenario — and the briefing's attention section BRF@v2; the register 50/0/0; the 49 stale-status candidates checked against the code and moved with evidence (16 implemented, 31 partial, 2 kept); seven units gain evidence and stay open) implemented and exercised on the demonstration (`evidence/cp6/act-b23.txt` — a material change routed to the decision owner, a review convened, the corridor stream backpressured, interrupted, resumed and closed incomplete at its publisher gap, the context answered partial while a projection was withdrawn, one graph revision committed with its retry and conflict, a branch added to the corridor scenario, the weekly briefing composed as v2 — ALL SCENES HELD); B23-F1 corrected by the forward migration 0085 (the context query's diagnostics obey the reader's purpose, clearance and audience); **B24** (0086, on `phase6-b24` stacked on #63: the attention completion — the timer host escalating under the attention agent's own session, the delivery port with receipts, bounded retries, in_app and the SYNTHETIC demo-mailbox (a real provider stays owner decision D6), the further materiality dimensions, the enforced overload rule (C3/C4 never held), the transparent rank and the rebalance, suppression approval under separation of duties, item delegation, dispositions and the queue evaluation, markers constraining the commitment and the run, the selected transformation plan executed by the extraction agent; rows implemented 1003 → 1013; seventeen units gain evidence and stay open) implemented and exercised on the demonstration (`evidence/cp6/act-b24.txt` — a C3 material change escalated by the agent's tick and delivered with receipts, an item held for capacity then elevated, a suppression approved by a second person, a delegation, the queue evaluated, a suspended source constraining the commitment and a run, a plan executed — ALL SCENES HELD, 63 checks); no deployment leg accepted** | none (read-only) | acceptance units for every capability group (`audit/acceptance-units/`), every `partial` row's remaining work verified against the code, the owner's four choices (R-4) recorded, `audit/SUMMARY.md` regenerated; no completion percentage |
...
Holds as they now stand: no waiver, no unchecked merge, no purchase, no cadence or budget change, no email; PortWatch activation proceeds within its granted permission; UN Comtrade authorized on 2026-09-12 within its permissions and the existing budgets and cadences, not yet activated (§10 of the source status); the `CorrectionApplied` consumer was delivered by CP-6 B1 and continues as required full-product work (P3).
